...
- **Bot mailbox** (e.g., `mailcode_bot@example.com`) — MailCode monitors its inbox. After the AI processes a task, results are sent back through this mailbox.
...
- **python3** (≥3.9)
- **Claude Code** or **OpenCode** (`claude` / `opencode` command must be in `PATH`)
...
| `mailcode chat` | Terminal interactive mode — talk to AI directly (no email) |
...
# Console output shows live events: 📬 email received → 🤖 invoking AI → ✅ reply sent
mailcode serve

# Dry-run mode (print emails only, don't invoke AI)
...
Talk to AI directly from the terminal without going through email:
...
Put `cwd: <path>` on the **first line** of your email body to start the AI subprocess in that directory — ideal for "AI, work on this project." In session mode, the cwd is **sticky**: subsequent emails in the same session reuse the directory until a new one is specified.
...
The cwd line is stripped from the body before invoking the AI, so it never pollutes the prompt.
...
- **Error notification** — if AI invocation or email sending fails, the configured recipient is notified automatically
